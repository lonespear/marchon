...
─────
  ...
  # Train silently, logging to archon.log
  python main.py --headless

  # Resume from a checkpoint
  python main.py --checkpoint checkpoints/archon_iter_0020.pt
...
class SharedStateHandler(logging.Handler):
    ...
    def __init__(self, shared_state):
        ...
        # Compact format for the narrow log panel
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            pass   # never let a log handler crash the trainer
...
def setup_logging(shared_state=None):
    ...
    if shared_state is not None:
        ...
        # Headless: also print WARNING+ to console
...
def main():
    parser = argparse.ArgumentParser(
        description="Archon — Chess RL Training"
    )
    parser.add_argument(
    ...
    if args.headless:
        # ── Headless mode: just train, no UI ──────────────────────────────────
        trainer.run(num_iterations=args.iterations)

    else:
        # ── Interactive mode: Flask in main thread, trainer in background ─────
        ...
        print(f"  On network → http://<pi-ip>:5000\n")
        try:
